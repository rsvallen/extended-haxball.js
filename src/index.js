...
} = require('@mertushka/node-datachannel/polyfill');
...
/**
 * @license HaxBall © Mario Carbajal - All rights reserved.
 * This file is a heavily minified and obfuscated version of the Haxball headless host library.
 * This comment aims to provide a comprehensive guide to its structure, functionality, and key components
 * for future developers and those interested in understanding the inner workings of Haxball.
 *
 * ## Table of Contents
 * 1.  Overall Architecture
 * 2.  Core Classes
 *     - A & L: Binary Data Handling
 *     - Pa: Room and Game State
 *     - Q: Game Simulation and Logic
 *     - w: Stadium Management
 *     - Aa: Player Management
 *     - Ha: Networking (WebSocket & WebRTC)
 *     - m: Action and Event System
 * 3.  Key Functions
 *     - F.fi: The Main Entry Point
 * 4.  Event Handling
 * 5.  Data Serialization
 *
 * ## 1. Overall Architecture
 *
 * The library is designed to run a Haxball room in a headless environment (e.g., a server).
 * It communicates with the main Haxball server and with clients (players) using a combination of WebSockets and WebRTC.
 *
 * The architecture can be broken down into these main parts:
 *
 * - **Initialization:** The `HBInit` promise and `HBLoaded` function are used to initialize the library. The main entry point is the `F.fi` function, which sets up the room based on a configuration object.
 * - **Networking:** The `Ha` class manages all network communication. It establishes a WebSocket connection to the Haxball server for signaling and then uses WebRTC for peer-to-peer communication with clients.
 * - **Game State Management:** The `Pa` class is the central hub for the game state. It holds information about the room, players, teams, stadium, and scores.
 * - **Game Simulation:** The `Q` class is responsible for the game's physics and logic. It updates the positions of discs (players and the ball), handles collisions, and detects goals.
 * - **Action and Event System:** The `m` class and its subclasses define all possible actions that can occur in the game (e.g., chat messages, player kicks, starting the game). These actions are processed and applied to the game state. The library also provides a rich set of event handlers (e.g., `onPlayerJoin`, `onGameStart`) for developers to hook into.
 * - **Data Serialization:** Custom classes `A` and `L` are used to efficiently serialize and deserialize game state and actions into binary data for network transmission.
 *
 * ## 2. Core Classes
 *
 * ### A & L: Binary Data Handling
 *
 * - **`A` (Writer):** This class is a custom implementation of a binary data writer. It's used to serialize game state and actions into an `ArrayBuffer`. It has methods to write different data types (e.g., `f` for Uint8, `w` for Int32, `g` for Float64, `Tb` for strings).
 * - **`L` (Reader):** This is the counterpart to `A`. It's a binary data reader that deserializes an `ArrayBuffer` back into JavaScript objects and primitive types. It has corresponding methods to read data (e.g., `C` for Uint8, `H` for Int32, `o` for Float64, `mb` for strings).
 *
 * ### Pa: Room and Game State
 *
 * This is one of the most important classes, as it holds the entire state of the room.
 *
 * - **Properties:**
 *   - `D`: A reference to the `Q` (game simulation) object when a game is in progress.
 *   - `ba`: An array of `Aa` (player) objects.
 *   - `ea`: The current `w` (stadium) object.
 *   - `Xd`: The room name.
 *   - `ab`: The score limit.
 *   - `Na`: The time limit.
 *   - `sc`: An array holding team color information.
 * - **Methods:**
 *   - `G(a)`: Serializes the entire room state into a binary buffer (`a`).
 *   - `R(a)`: Finds a player by their ID (`a`).
 *   - `Hc(a, b, c)`: Changes a player's (`b`) team to a new team (`c`), triggered by a player (`a`).
 *   - `ei(a)`: Starts a new game, triggered by a player (`a`).
 *
 * ### Q: Game Simulation and Logic
 *
 * This class handles the game's physics and logic when a match is active.
 *
 * - **Properties:**
 *   - `ia`: A `Qa` object that holds the state of all discs (players and the ball) in the game.
 *   - `zb`, `vb`: The scores for the red and blue teams, respectively.
 *   - `Lb`: The current game time.
 *   - `ab`, `Na`: The score and time limits for the current match.
 *   - `ea`: A reference to the current stadium.
 * - **Methods:**
 *   - `Ba(a)`: The main update loop for the game. It's called on every "tick" to update disc positions, handle collisions, and apply forces.
 *   - `eh(a)`: Initializes the game simulation with the room state (`a`).
 *   - `$e(a)`: Sets up a player's disc (`a`) when they join a team.
 *   - `af()`: Resets the positions of all discs (e.g., after a goal).
 *
 * ### w: Stadium Management
 *
 * The `w` class represents a Haxball stadium. It holds all the geometric and physical properties of the playing field.
 *
 * - **Properties:**
 *   - `oa`: The stadium name.
 *   - `v`, `I`, `Y`, `Sa`, `A`: Arrays containing the stadium's vertices, segments, planes, goals, and discs.
 *   - `Mb`: The physics properties for players in this stadium.
 * - **Methods:**
 *   - `G(a)`: Serializes the stadium data.
 *   - `oi(a)`: Deserializes stadium data from a binary buffer.
 *   - `kh(a)`: Parses a stadium from a JSON5 string (HBS file format).
 *   - `Gd()`: Returns a list of default built-in stadiums.
 *
 * ### Aa: Player Management
 *
 * The `Aa` class represents a player in the room.
 *
 * - **Properties:**
 *   - `ma`: The player's unique ID.
 *   - `oa`: The player's name.
 *   - `ja`: The player's team (`u.fa` for red, `u.ta` for blue, `u.na` for spectators).
 *   - `Ub`: A boolean indicating if the player is an admin.
 *   - `country`: The player's country code.
 *   - `N`: A reference to the player's `xa` (disc) object in the game simulation.
 * - **Methods:**
 *   - `P(a)`: Serializes the player's data.
 *
 * ### Ha: Networking (WebSocket & WebRTC)
 *
 * This class is responsible for all network communication.
 *
 * - **Methods:**
 *   - `ce(a)`: Connects to the Haxball server to get a room URL and token.
 *   - `$g(...)`: Handles an incoming WebRTC offer from a client, sets up a peer connection, and sends back an answer.
 *   - `pc(...)`: Sends a message to a client via WebRTC.
 *
 * ### m: Action and Event System
 *
 * The `m` class is the base class for all actions in the game. Each action is a separate subclass of `m`.
 *
 * - **Examples of Action Subclasses:**
 *   - `la`: A chat message.
 *   - `S`: Kicking a player.
 *   - `aa`: Setting a player's team.
 *   - `ma`: Starting a game.
 *   - `na`: Stopping a game.
 *   - `V`: Changing the stadium.
 * - **Methods:**
 *   - `apply(a)`: Applies the action to the game state (`a`).
 *   - `P(a)`: Serializes the action.
 *   - `W(a)`: Deserializes the action.
 *
 * ## 3. Key Functions
 *
 * ### F.fi: The Main Entry Point
 *
 * This is the function that you call to create and initialize a headless room. It takes a configuration object as an argument and returns an API object for interacting with the room.
 *
 ...
 *   - `token`: Your Haxball headless host token.
 * - **Returned API Object (`J`):**
 *   - `sendChat(message, [playerID])`: Sends a chat message.
 *   - `setPlayerAdmin(playerID, isAdmin)`: Sets a player's admin status.
 *   - `kickPlayer(playerID, reason, ban)`: Kicks or bans a player.
 *   - `startGame()`: Starts the game.
 *   - `stopGame()`: Stops the game.
 *   - `setCustomStadium(stadiumJSON)`: Loads a custom stadium.
 *   - `...and many more.`
 *
 * ## 4. Event Handling
 *
 * The `F.fi` function also sets up a series of event handlers that you can define in your code. These are your main tools for creating room logic.
 *
 * - **Example Event Handlers:**
 *   - `onPlayerJoin(player)`: Called when a player joins the room.
 *   - `onPlayerLeave(player)`: Called when a player leaves.
 *   - `onPlayerChat(player, message)`: Called when a player sends a chat message.
 *   - `onGameStart(byPlayer)`: Called when the game starts.
 *   - `onTeamGoal(team)`: Called when a team scores a goal.
 *   - `onGameTick()`: Called on every game simulation step.
 *
 * ## 5. Data Serialization
 *
 * The library uses a highly optimized binary serialization format to minimize network traffic. Almost every object that needs to be sent over the network has a `G(writer)` method for serialization and a corresponding `pa(reader)` or `W(reader)` method for deserialization.
 *
 * For example, when a new player joins, the entire game state (`Pa`) is serialized using its `G` method and sent to the client. The client then deserializes it to reconstruct the current state of the game.
 *
 */
('use strict');
(function (va) {
  ...
  class A {
    ...
    Ca(a) {
      null == a ? this.Da(0) : (this.Da(A.Ad(a) + 1), this.ie(a));
    }
    Uf(a) {
  ...
  class T {
    ...
    static Pg(a) {
      return T.zf(a, 'GET', null);
    }
    static Rg(a) {
      return T.Pg(a).then(function (b) {
        let c = b.error;
    ...
    static Hh(a, b, c) {
      return T.zf(a, 'POST', b, c);
    }
    static Ih(a, b, c) {
      return T.Hh(a, b, c).then(function (d) {
        let e = d.error;
  ...
  class E {
    ...
    bb(a) {
      ...
      a > E.jg && a < E.ig && (this.Ha = 1 / Math.tan(a / 2));
    }
    Nb() {
      if (0 == 0 * this.Ha) {
        var a = this.K.a,
  ...
  class q {
    ...
    static lg(a, b) {
      return a instanceof b ? !0 : b.yd ? q.De(q.Yf(a), b) : !1;
    }
    static l(a, b) {
      if (null == a || q.mg(a, b)) return a;
      throw r.s('Cannot cast ' + Oa.xe(a) + ' to ' + Oa.xe(b));
    }
    static Ee(a) {
  ...
  class Pa {
    ...
    ai(a, b, c, d) {
      ...
      0 > d ? (d = 0) : 100 < d && (d = 100);
      this.Pd = this.fc * d;
      Za.sa(this.hh, a, this.Xc, this.fc, d);
    }
  }
  class Za {
    static sa(a, b, c, d, e) {
  ...
  class $a {
    constructor() {}
    yg() {
      this.oa = R.qb(this.oa, 40);
      this.ib = R.qb(this.ib, 3);
    }
    G(a) {
      this.yg();
      a.ra = !0;
  ...
  class Q {
    ...
    $e(a) {
      if (a.ja == u.na) a.N = null;
        ...
        a.Jb = 0;
        var b = a.N;
        null == b && ((b = new xa()), (a.N = b), this.ia.A.push(b));
        var c = this.ea.Mb;
        b.T = 0;
        b.M = c.M;
        b.L = c.L;
    ...
    Ba(a) {
      if (0 < this.Xa) 120 > this.Xa && this.Xa--;
        ...
        if (0 == this.$a) {
          for (a = 0; a < b.length; )
            (c = b[a]), ++a, null != c.N && (c.N.c = 39 | this.Nd.ih);
          b = this.ia.A[0].u;
  ...
  class Ga {
    static jh() {
      if (null != Ga.Ud) return Ga.Ud;
      Ga.Ud = new Promise(function (a, b) {
        var c = grecaptcha;
        null != c
          ? a(c)
  ...
  class Ha {
    ...
    ce(a) {
      ...
      null == a && (a = '');
      let d = this;
      T.Ih(this.di, 'token=' + this.mc + '&rcr=' + a, T.hg)
        .then(function (e) {
          switch (e.action) {
            case 'connect':
              c(e);
    ...
    $g(a, b, c, d, e, f) {
      if (16 <= this.xb.size) this.oc(a, 4104);
        ...
        if (
          null != this.Ne &&
          ((k = new L(e.j)), (k.a = e.a), (e = this.Ne(b, k)), 1 == e.Kd)
        ) {
          this.oc(a, e.reason);
          return;
        }
        var l = new sa(a, this.Md, this.zg);
        f && (l.Pe = 2500);
        l.Rb = g;
        l.hb = b;
        this.xb.set(a, l);
  ...
  class w {
    ...
    oi(a) {
      ...
      this.I = [];
      c = a.C();
      for (d = 0; d < c; ) ++d, (e = new E()), e.pa(a, this.v), this.I.push(e);
      this.Y = [];
      c = a.C();
      for (d = 0; d < c; ) ++d, (e = new K()), e.pa(a), this.Y.push(e);
      this.Sa = [];
      c = a.C();
      for (d = 0; d < c; ) ++d, (e = new ya()), e.pa(a), this.Sa.push(e);
      this.A = [];
      c = a.C();
      for (d = 0; d < c; ) ++d, (e = new ha()), e.pa(a), this.A.push(e);
      this.Ta = [];
      c = a.C();
      for (d = 0; d < c; ) ++d, (e = new Ia()), e.pa(a), this.Ta.push(e);
      this.Pb = b();
      this.Ib = b();
      this.Nb();
      if (!this.Rf()) throw r.s(new Ja('Invalid stadium'));
    }
    Rf() {
          ...
          if (((t = q.l(x, Array)), null != t))
            for (x = 0; x < t.length; ) {
              ...
              try {
                ...
                throw r.s(new Ja('Error in "' + l + '" index: ' + k.length));
              }
            }
      }
      let d = JSON5.parse(a);
    ...
    static pa(a) {
      var b = a.C();
      return 255 == b ? ((b = new w()), b.oi(a), b) : w.Gd()[b];
    }
    static Gd() {
      if (null == w.wa) {
        w.wa = [];
    ...
    static Sc(a) {
      ...
      if ('string' == typeof a) return Oa.parseInt('0x' + Oa.xe(a));
      if (a instanceof Array)
  ...
  class F {
    ...
    static fi(a) {
      ...
      if (F.Ff) throw r.s("Can't init twice");
      F.Ff = !0;
      ...
      let t = g('playerName', 'Host'),
        x = k('noPlayer', !1),
        B = q.l(k('maxPlayers', 12), za),
        N = g('password', null),
        ta = g('token', null),
        ub = k('geo', null),
        Ka = F.Se;
      if (null != ub && ((Ka = ia.Te(ub)), 3 < Ka.ib.length))
      ...
      D.kf = function (h) {
        null != y.R(h) && D.nc(S.V(h, 'Bad actor', !1));
      };
      D.yh = function (h, n) {
        let v = n.mb();
      ...
      D.zh = function (h) {
        null != y.R(h) && D.nc(S.V(h, null, !1));
      };
      D.Yc = function (h) {
        h = F.uh(h, null != D.lb);
        F.Vh = '<p>Room Link: <a href="' + h + '">' + h + '</a></p>';
        {
          let n = J.onRoomLink;
          null != n && n(h);
        }
      ...
      y.Eh = function (h) {
        var n = D.wb.get(h.ma),
          v = null,
          G = null;
        null != n && ((v = n.Ig), (G = n.jc.hb));
        n = v;
        v = J.onPlayerJoin;
        null != v && ((h = f(h)), (h.auth = n), (h.conn = G), v(h));
      };
      y.Mf = function () {
        {
  ...
  class ia {
    ...
    static Sg() {
      return T.Rg(wa.Ke + 'api/geo').then(function (a) {
        return ia.Te(a);
      });
    }
  }
  ...
  class sa {
    constructor(a, b, c) {
      ...
      this.Ld = [];
      this.jb = [];
      this.qa = new RTCPeerConnection({ iceServers: b }, sa.Fg);
      let d;
    ...
    gi() {
      ...
      this.ee = setTimeout(hb(this, this.dh), a);
    }
    async Gg(a, b) {
  ...
  class Ua extends La {
    ...
    Aa(a) {
      ...
      this.Wa = new Ha(this.vh, a.iceServers, kb.channels, a.Gi);
          ...
          try {
            if (-1 != e.Ga.indexOf(b)) {
              ...
              Sa.sa(
                e.yh,
                g,
                new L(new DataView(d.buffer, d.byteOffset, d.byteLength), !1)
              );
              b.cd = !0;
              e.$h(b);
            }
    ...
    Mh(a, b) {
      ...
      if (null != e) {
        var f = b.He.get(e);
        null == f && ((f = new Ra(e.pd, e.wd)), b.He.set(e, f));
        if (!f.Qf()) throw r.s(3);
      }
      e = this.da + this.Qb;
      f = this.da;
      ...
      a.B = b.S;
      a.eb = f;
      a.dg(this.Fa) && (this.Bf(a), this.hd(this.Od(a), 1));
    }
  }
  class Ea extends m {
    constructor() {
    ...
    apply(a) {
      let b = a.R(this.B);
      null != b && this.re != b.ne && ((b.ne = this.re), Na.sa(a.Ki, b));
    }
    P(a) {
      a.f(this.re ? 1 : 0);
    }
  ...
  class Y extends m {
    ...
    apply(a) {
      0 == this.B && Za.sa(a.Mi, this.Pa, this.color, this.style, this.we);
    }
    P(a) {
      a.Tb(R.qb(this.Pa, 1e3));
      a.w(this.color);
      a.f(this.style);
      a.f(this.we);
    }
    ...
    apply(a) {
      if (a.za(this.B)) {
        ...
        null != c &&
          0 != c.ma &&
          c.Ub != this.sd &&
          ((c.Ub = this.sd), null != a.sf && a.sf(b, c));
      }
    }
    P(a) {
      a.w(this.Fb);
  ...
  class Fa extends m {
    ...
    W(a) {
      this.Ea = a.Ya();
      null != this.Ea && (this.Ea = R.qb(this.Ea, 2));
    }
  }
  class aa extends m {
    constructor() {
    ...
    W(a) {
      this.Fb = a.H();
      a = a.Vd();
      this.se = 1 == a ? u.fa : 2 == a ? u.ta : u.na;
    }
    static V(a, b) {
      let c = new aa();
      c.Fb = a;
  ...
  class V extends m {
    ...
    W(a) {
      ...
      this.vd = w.pa(new L(new DataView(a.buffer, a.byteOffset, a.byteLength)));
    }
    static V(a) {
      let b = new V();
      b.vd = a;
  ...
  class pa extends m {
    ...
    apply(a) {
      if (a.za(this.B)) {
        var b = a.md;
        a.md = this.newValue;
        b != this.newValue && Sa.sa(a.ii, a.R(this.B), this.newValue);
      }
    }
    P(a) {
  ...
  class oa extends m {
    ...
    apply(a) {
      ...
      if (null != b && a.za(this.B)) {
        ...
        d != this.wc && jb.sa(a.Ch, c, this.wc, e);
      }
    }
    P(a) {
      a.f(this.wc ? 1 : 0);
  ...
  class la extends m {
    ...
    apply(a) {
      let b = a.R(this.B);
      null != b && Sa.sa(a.Ji, b, this.Pa);
    }
    P(a) {
      a.Tb(R.qb(this.Pa, 140));
    }
    W(a) {
      this.Pa = a.mb();
    ...
    apply(a) {
      if (0 != this.ma && a.za(this.B)) {
        ...
        if (null != b) {
          ...
          Za.sa(a.Fh, b, this.rb, this.od, c);
        }
      }
    }
    P(a) {
      null != this.rb && (this.rb = R.qb(this.rb, 100));
      a.w(this.ma);
      a.Ca(this.rb);
      a.f(this.od ? 1 : 0);
    }
  ...
  class M extends m {
    ...
    static ag(a, b, c) {
      ...
      for (b = d.O.length; a < b; ) {
        c = a++;
        var e = d.O[c];
        null != e && ((M.Xf[0] = e), (d.O[c] = M.Xf[0]));
      }
      a = 0;
      for (b = d.ya.length; a < b; )
        (c = a++),
          (e = d.ya[c]),
          null != e && ((M.$f[0] = e), (d.ya[c] = M.$f[0]));
      return d;
    }
  }
  class ea extends m {
    ...
    apply(a) {
      a.za(this.B) && a.ai(a.R(this.B), this.min, this.rate, this.ke);
    }
    P(a) {
      a.w(this.min);
      a.w(this.rate);
  ...
  kb.b = !0;
  lb.b = !0;
  Object.assign(lb.prototype, { h: lb });
  I.b = !0;
  ...
  Za.b = !0;
  Ra.b = !0;
  Object.assign(Ra.prototype, { h: Ra });
  ia.b = !0;
  ...
  u.ta.Zc = u.fa;
  q.pg = {}.toString;
  sa.Fg = { mandatory: { OfferToReceiveAudio: !1, OfferToReceiveVideo: !1 } };
  ...
  T.hg = 'application/x-www-form-urlencoded';
  wa.Ke = 'https://www.haxball.com/rs/';
  wa.Md = [{ urls: 'stun:stun.l.google.com:19302' }];
  F.Se = new ia();
  F.Ff = !1;
  Q.jf = (function () {
    let a = [];
  ...
  Ea.U = m.Z({ ca: !1, delay: !1, td: { pd: 2, wd: 1e4 } });
  Y.U = m.Z({ ca: !1, delay: !1, td: { pd: 10, wd: 900 } });
  Da.U = m.Z({ ca: !1, delay: !1 });
  U.U = m.Z({ ca: !1, delay: !1 });
  ba.U = m.Z({ ca: !1, delay: !1 });
  Fa.U = m.Z({ ca: !1, delay: !1 });
  aa.U = m.Z({ ca: !1, delay: !1 });
  V.U = m.Z({ ca: !1, delay: !1, td: { pd: 10, wd: 2e3 } });
  qa.U = m.Z({ ca: !1, delay: !1 });
  pa.U = m.Z({ ca: !1, delay: !1 });
  Z.U = m.Z({ ca: !1, delay: !1 });
  fa.U = m.Z({ ca: !1, delay: !1 });
  oa.U = m.Z({});
  la.U = m.Z({ ca: !1, delay: !1, td: { pd: 10, wd: 900 } });
  Ca.U = m.Z({});
  Ba.U = m.Z({ ca: !1, delay: !1 });
  S.U = m.Z({ ca: !1, delay: !1 });
  da.U = m.Z({ ca: !1, delay: !1 });
  M.U = m.Z({ ca: !1, delay: !1 });
  M.Xf = new Float32Array(1);
  M.$f = new Int32Array(1);
  ea.U = m.Z({ ca: !1, delay: !1 });
  ma.U = m.Z({ ca: !1, delay: !1 });
  na.U = m.Z({ ca: !1, delay: !1 });
  ca.U = m.Z({ ca: !1, delay: !1 });
  E.jg = 0.17435839227423353;
  E.ig = 5.934119456780721;
  F.sh();
})(
